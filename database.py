...
def get_books_by_author(author: str) -> Optional[Dict]:
...
def get_books_by_title(title: str) -> Optional[Dict]:
    """Get a specific book by title."""
    conn = get_db_connection()
    books = conn.execute('SELECT * FROM books WHERE title LIKE ?', (f'%{title}%',)).fetchall()
    conn.close()
    return [dict(book) for book in books] if books else None
...
def delete_borrow_record(patron_id: str, book_id: int, borrow_date: datetime, due_date: datetime) -> bool:
    """Delete a borrow record from the database."""
    conn = get_db_connection()
    try:
        conn.execute('''
            DELETE FROM borrow_records (patron_id, book_id, borrow_date, due_date)
            VALUES (?, ?, ?, ?)
        ''', (patron_id, book_id, borrow_date.isoformat(), due_date.isoformat()))
        conn.commit()
        conn.close()
...
def get_borrow_records(patron_id: str):
    """Update the return date for a borrow record."""
    conn = get_db_connection()
    records = conn.execute('''
        SELECT COUNT(*) as count FROM borrow_records 
